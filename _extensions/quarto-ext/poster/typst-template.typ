#let poster(
  ...
  // Lab logo width.
  lab_logo_width: "3",  // Reduced from 5 to 3
  ...
  univ_logo_scale: "55",  // Reduced from 80 to 55

  // University logo's column size (in in).
  univ_logo_column_size: "3",  // Reduced from 4 to 3

  // Title and authors' column size (in in).
  title_column_size: "12",  // Reduced from 18 to 12

  // Poster title's font size (in pt).
  title_font_size: "20",  // Further reduced from 24 to 20

  // Authors' font size (in pt).
  authors_font_size: "16",  // Further reduced from 18 to 16

  // Footer's URL and email font size (in pt).
  footer_url_font_size: "13",  // Further reduced from 15 to 13

  // Footer's text font size (in pt).
  footer_text_font_size: "18",  // Further reduced from 20 to 18

  // The poster's content.
  body,
) = {
  // Set the body font with smaller size
  set text(font: "STIX Two Text", size: 6.5pt)  // Further reduced from 7pt to 6.5pt
  ...
  title_font_size = int(title_font_size) * 0.85pt  // Further reduced to 85% (from 90%)
  authors_font_size = int(authors_font_size) * 0.85pt  // Further reduced to 85%
  num_columns = int(num_columns)
  footer_url_font_size = int(footer_url_font_size) * 0.85pt  // Further reduced to 85%
  footer_text_font_size = int(footer_text_font_size) * 0.85pt  // Further reduced to 85%
  ...
  set page(
    ...
    height: height,
    margin: 
      (top: 1.25cm, left: 1.25cm, right: 1.25cm, bottom: 1.25cm)  // Increased margins to reduce content area
  )
  ...
  // Configure headings with smaller sizes
  ...
  show heading: it => locate(loc => {
    ...
    set text(9pt, weight: 400)  // Further reduced from 10pt to 9pt
    if it.level == 1 [
      // First-level headings are centered smallcaps.
      #set align(center)
      #set text({ 12pt })  // Further reduced from 14pt to 12pt
      #show: smallcaps
      #v(20pt, weak: true)  // Further reduced from 23pt to 20pt
      #block(
        width: 100%,
        fill: theme_color, // Using original theme color (not darkened)
        inset: (x: 7pt, y: 4pt),  // Reduced from 12pt,8pt to 7pt,4pt
        radius: 2pt,  // Reduced from 4pt to 2pt
        [
          #set text(fill: white)  // Set text color to white
          #it.body
        ]
      )
      #v(9pt, weak: true)  // Reduced from 16pt to 9pt
    ] else if it.level == 2 [
      // Second-level headings with width that fits the content
      #set text({ 10pt })  // Further reduced from 11pt to 10pt
      #v(12pt, weak: true)  // Further reduced from 14pt to 12pt
      #align(left)[
        #box(
          fill: theme_color.lighten(25%), // Lighter theme color
          inset: (x: 6pt, y: 4pt),  // Reduced from 10pt,6pt to 6pt,4pt
          radius: 2pt,  // Reduced from 3pt to 2pt
          [
            #set text(fill: white)  // Set text color to white
            #it.body
          ]
      ...
      #v(7pt, weak: true)  // Reduced from 12pt to 7pt
    ] else if it.level == 3 [
      // Third-level headings with simpler styling
      #set text(size: 8pt, weight: "bold")  // Further reduced from 9pt to 8pt
      #v(8pt, weak: true)  // Further reduced from 9pt to 8pt
      #it.body
      #v(4pt, weak: true)  // Reduced from 8pt to 4pt
    ] else [
      // Level 4+ headings
      _#(it.body):_
    ]
    ...
    align(center, 
      stack(
        ...
        block(
          ...
          inset: 15pt,  // Reduced from 20pt to 15pt
          {
            set text(fill: white)  // Set text color to white
  ...
  block(width: 100%, height: 100% - 3.5cm, {  // Further reduced from 4cm to 3.5cm
    // Main content area with fixed height to leave room for footer
    columns(num_columns, gutter: 0.8em, {  // Further reduced from 1em to 0.8em
      set par(justify: true, first-line-indent: 0em)
      show par: set block(spacing: 0.25em)  // Further reduced from 0.3em to 0.25em
    ...
    block(
      ...
      align(center, {
        block(
          ...
          inset: 6pt,  // Reduced from 11pt to 6pt
          radius: 3pt,  // Reduced from 5pt to 3pt
          {
            // Add lab logo to footer if provided
            if lab_logo != "" {
              image(lab_logo, width: int(lab_logo_width) * 0.4cm)  // Reduced from 0.7cm to 0.4cm
              v(0.2em)  // Reduced from 0.4em to 0.2em
            }
            ...
            if logos.len() > 0 {
              grid(
                ...
                gutter: 9pt,  // Reduced from 16pt to 9pt
                ..logos.map(logo => {
                  image(logo, height: 0.9cm)  // Reduced from 1.6cm to 0.9cm
                })
              )
            }
          }
