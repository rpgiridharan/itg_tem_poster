...
### Growth rate, $\gamma$
All equations are damped by $D\nabla_\perp^2 X_i + \nu\partial_\parallel^2X_i$. We use $D=5e-3$, $\nu=3e-2$

- $\gamma(k_x, k_y)$ is symmetrical about $k_x$ and peaks at around $k_y\approx 1$.
- There is a max value of kz, depending on $\nu$, upto which $\gamma$ is positive$. $k_z=1$ is chosen later on since it is small enough for most values of $\nu$ used.

::: {layout="[[1,1]]"}
![$\gamma(k_x,k_y)$](images/gamkxky_itg_tem_simple.png)

![$\gamma(k_y,k_{z,i})$](images/gamkykz_itg_tem_simple.png)
:::
...
We ignore trapped electrons and set $\kappa_n=0$ to obtain a model as in [ivanov, 2020; ivanov, 2022] and simulate the system using a pseudospectral code. 
